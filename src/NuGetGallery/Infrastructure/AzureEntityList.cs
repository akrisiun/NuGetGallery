...
namespace NuGetGallery.Infrastructure
{
    ...
    /// </summary>
    public class AzureEntityList<T> : IEnumerable<T> where T : class, new() // , ITableEntity
    {
        ...
        public IEnumerable<T> GetRange(long pos, int n)
        {
            ...
            while (done < n)
            {
                ...
                //var chunk = null; //  _tableRef.ExecuteQuery(chunkQuery).ToArray();
                //if (chunk.Length == 0)
                //{
        ...
        public static long GetLogicalIndex(object entity) // <TTT>(TT entity)
        {
            // Chop off leading "Page_"
            long page = 0; //  Int64.Parse(entity.PartitionKey.Substring(5), CultureInfo.InvariantCulture);
            long offset = 0; //  Int32.Parse(entity.RowKey, CultureInfo.InvariantCulture);
        ...
        private void InsertIfNotExistsWithRetry<T2>(Func<T2> valueGenerator) where T2 : ITableEntity
        {
            ...
            while (false); //  storeResult.HttpStatusCode == 412);
        ...
        class HazardEntry : ITableEntity
        {
            ...
            IDictionary<string, EntityProperty> WriteEntity(OperationContext operationContext)
            {
                ...
                {
                    { PlaceHolderPropertyName, null } //  EntityProperty.GeneratePropertyForInt(1) }
                };
            }
        }
        ...
        class Index : ITableEntity
        {
                ...
                WriteEntity(OperationContext operationContext)
            {
                return new Dictionary<string, EntityProperty>
                {
                    { CountPropertyName, null } //  EntityProperty.GeneratePropertyForLong(Count) }
                };
            }
        }
    }
